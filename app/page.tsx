  ...
  return (
    <div className="mt-6 flex flex-wrap items-center gap-4">
      <div className="flex -space-x-2">
        {[1, 2, 3, 4].map((item) => (
          <div key={item} className="h-11 w-11 overflow-hidden rounded-full border-2 border-[#071321] bg-[#102740]">
            <Image src="/placeholder-avatar.svg" alt="Professional" width={44} height={44} className="h-full w-full object-cover" />
          </div>
        ))}
        <div className="flex h-11 w-11 items-center justify-center rounded-full border-2 border-[#071321] bg-[#1c3349] text-xs font-semibold text-[#f7e8c6]">
          +2K
        </div>
      </div>
      <p className="max-w-xs text-[1.03rem] leading-7 text-[#f7e8c6]/88">
...
function TalentPassportCard() {
  return (
    <article className="fa-float relative origin-center rotate-[2.5deg] rounded-[36px] border border-[#f7e8c6]/90 bg-[#f7e8c6] p-3 shadow-[0_38px_110px_rgba(2,9,24,0.62)] transition duration-500 hover:-translate-y-1 hover:rotate-[4.2deg] sm:rotate-[4.5deg]">
  ...
  return (
    <main className="bg-[#0B111D] text-[#f7e8c6]">
      ...
      <section className="relative isolate overflow-hidden border-b border-[#2bd7ef]/12">
        ...
        <div className="nav-fade relative mx-auto grid max-w-7xl items-center gap-10 px-6 pb-10 pt-8 sm:px-8 lg:min-h-[calc(100vh-80px)] lg:grid-cols-[44fr_56fr] lg:px-12 lg:py-6">
          <div className="max-w-xl">
            ...
            <div className="mt-7 flex flex-col gap-3 sm:flex-row">
              <Link
                href="/builder"
                className="inline-flex items-center justify-center rounded-full bg-[#aff546] px-7 py-3 text-sm font-semibold text-[#071321] transition duration-300 hover:-translate-y-0.5 hover:bg-[#9fea37]"
              >
      ...
      <section className="bg-[#f7e8c6] text-[#071426]">
        <div className="mx-auto grid max-w-7xl gap-8 px-6 py-11 sm:px-8 lg:grid-cols-3 lg:gap-0 lg:px-12">
          {featureCards.map((feature, index) => {
            ...
            return (
              ...
              >
                ...
                <h3 className="mt-5 font-serif text-4xl font-semibold leading-tight text-[#071321]">{feature.title}</h3>
                <p className="mt-3 text-lg leading-8 text-[#071321]/86">{feature.description}</p>
                <Link href="#" className={`mt-5 inline-flex items-center text-lg font-medium transition hover:translate-x-0.5 ${linkClass}`}>
                  Learn more
