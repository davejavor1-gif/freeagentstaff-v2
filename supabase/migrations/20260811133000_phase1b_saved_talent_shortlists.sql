...
begin
  ...
  with ins as (
    ...
    on conflict (employer_user_id, talent_user_id) do nothing
    returning id, created_at
  )
  select
    coalesce((select id from ins), s.id),
    coalesce((select created_at from ins), s.created_at),
    exists(select 1 from ins)
  into v_saved_id, v_saved_at, v_new_insert
  ...
  return query
  select true, (not v_new_insert), v_saved_id, v_saved_at;
end
$$;
