-- Treat PostgreSQL role setting 'none' (direct postgres connections) as the
-- current_user, so trusted SQL/service_role writers still work, while
-- PostgREST SET ROLE authenticated continues to freeze billing fields.
...
begin
  if v_role is null or v_role in ('', 'none') then
    v_role := current_user;
  end if;
