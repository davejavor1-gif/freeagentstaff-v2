...
-- Make legal-consent evidence immutable once first recorded: NULL -> value is allowed,
-- but an already-recorded value cannot be changed or cleared by ordinary profile updates.
-- Re-creates the current canonical profiles_guard_protected_fields() function (matching
-- the final version in supabase/initial-schema.sql) with the new immutability checks added.
