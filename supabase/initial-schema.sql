...
returns table (
  slug text, availability text, opportunity_status text, experience_years integer,
  focus_area text, top_strength text, skills text[], passions text[], languages text[],
  location text, name text, title text, summary text, bio text,
  career_journey jsonb, education text, education_entries jsonb, salary_expectation text
)
language sql security definer stable set search_path = public, pg_temp
as $$
  select p.slug, p.availability, p.opportunity_status, p.experience_years,
    p.focus_area, p.top_strength, p.skills, p.passions, p.languages, p.location,
    p.name, p.title, p.summary, p.bio, p.career_journey,
    p.education, p.education_entries, p.salary_expectation
  from public.profiles p
  where p.account_type = 'talent'
...
language plpgsql security definer stable set search_path = public, pg_temp as $$
declare v_uid uuid := auth.uid(); v_talent uuid; v_intro public.employer_introduction_requests; v_connection public.employer_talent_connections;
begin
  if v_uid is null then raise exception 'not_signed_in' using errcode = '42501'; end if;
  select user_id into v_talent from public.profiles where account_type = 'talent' and slug = p_talent_slug;
  if v_talent is null then raise exception 'private_access_unavailable' using errcode = '42501'; end if;
