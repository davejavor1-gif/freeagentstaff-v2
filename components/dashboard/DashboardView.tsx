...
import EmployerConnectionsSection from "@/components/connections/EmployerConnectionsSection";
  ...
  return <div className="mx-auto grid max-w-[1500px] gap-4 lg:grid-cols-[240px_minmax(0,1fr)] lg:items-start xl:gap-6">
    ...
    <main className="min-w-0 space-y-5">
      {activeSection === "overview" ? <>
        ...
        {props.isVerifiedEmployer ? <section className="rounded-[24px] border border-[#cda64d]/45 bg-[#f7e8c6] p-6 text-[#08111F] shadow-[0_18px_45px_rgba(6,16,33,0.16)] sm:p-7"><p className="text-[10px] font-black uppercase tracking-[0.22em] text-[#08798a]">Employer metrics</p><div className="mt-6 grid divide-y divide-[#08111F]/12 border-t border-[#08111F]/12 pt-2 sm:grid-cols-2 sm:divide-x sm:divide-y-0 xl:grid-cols-4"><EmployerMetric label="Saved talent" value={summary?.savedTalentCount ?? 0} detail="People you bookmarked" icon={Bookmark} /><EmployerMetric label="Introductions" value={summary?.pendingIntroductionRequests ?? 0} detail="Waiting for a response" icon={Handshake} /><EmployerMetric label="Connections" value={summary?.activeConnections ?? 0} detail="Talent you can contact" icon={Users} /><EmployerMetric label="Shortlists" value={summary?.activeShortlists ?? 0} detail="Candidates you are considering" icon={ClipboardList} /></div></section> : null}
      ...
      {activeSection === "connections" ? <EmployerConnectionsSection /> : null}
