...
function TalentView(props: Props) {
  ...
  return <>
    ...
    <div className="space-y-6">
      ...
      <section className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4"><DashboardMetricCard compact href="/dashboard#connections" inlineAction label="Introductions" value={summary?.pendingIntroductionRequests ?? 0} detail="Waiting for your response" icon={Link2} valueClassName="text-[#08111F]" /><DashboardMetricCard compact href="/dashboard#connections" inlineAction label="Connections" value={summary?.activeConnections ?? 0} detail="Active employer relationships" icon={Link2} valueClassName="text-[#08111F]" />{props.isProTalent && summary?.proAnalytics ? <><DashboardMetricCard compact label="Profile views" value={summary.proAnalytics.profileViews} detail="Verified engagement" icon={Activity} valueClassName="text-[#08111F]" /><DashboardMetricCard compact label="Employer saves" value={summary.proAnalytics.employerSaves} detail="Saved by employers" icon={Sparkles} valueClassName="text-[#08111F]" /></> : <><DashboardMetricCard compact label="Profile views" value="" detail="Unlock employer views" icon={Activity} middle={<BillingButton action="checkout" plan="free_agent_pro" className="dashboard-upgrade-pill">Upgrade to Pro <span aria-hidden="true">→</span></BillingButton>} /><DashboardMetricCard compact label="Employer saves" value="" detail="Unlock employer saves" icon={Sparkles} middle={<BillingButton action="checkout" plan="free_agent_pro" className="dashboard-upgrade-pill">Upgrade to Pro <span aria-hidden="true">→</span></BillingButton>} /></>}</section>
