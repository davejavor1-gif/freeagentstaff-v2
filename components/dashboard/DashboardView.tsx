...
function EmployerView(props: Props) {
  ...
  return <>
    <DashboardHeader audience="employer" name={props.name} status={props.verificationLabel}>
      ...
      {canWork ? <BillingButton action="portal" className="dashboard-action dashboard-action-outline">Manage subscription</BillingButton> : null}
    </DashboardHeader>
    ...
    <div className="space-y-6">
      ...
      {canWork ? <>
        ...
        <div className="grid items-stretch gap-4 lg:grid-cols-[minmax(0,2fr)_minmax(220px,1fr)]">
          ...
          <DashboardPanel title="Sign out" eyebrow="Account"><p className="mt-4 text-sm text-[#08111F]/60">Finished for now?</p><button type="button" onClick={props.signOut} className="dashboard-action dashboard-action-outline">Sign out <span aria-hidden="true">→</span></button></DashboardPanel>
        </div>
      </> : null}
    </div>
  </>;
...
function TalentView(props: Props) {
  ...
  return <>
    <DashboardHeader audience="talent" name={props.name} status={status}>
      <DashboardAction href={props.talentPassportHref}>View my card</DashboardAction>
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-6 xl:grid-cols-[1.1fr_0.9fr]"><DashboardPanel title="Your FreeAgent Card" eyebrow="Professional identity" action={<div className="flex flex-wrap gap-3"><DashboardAction href="/builder">{props.hasCompletedTalentCard ? "Edit card" : "Create card"}</DashboardAction><DashboardAction href={props.talentPassportHref} variant="outline">View Passport</DashboardAction></div>}><div className="mt-6 border-t border-[#08111F]/15 pt-5">{props.dashboardTalentProfile ? <TalentCard profile={props.dashboardTalentProfile} href={props.talentPassportHref} className="max-w-[310px]" /> : <p className="text-base text-[#08111F]/60">Build your profile to see your FreeAgent Card.</p>}</div></DashboardPanel><DashboardPanel title="Recent activity" eyebrow="Your network" action={<Link href="/connections" className="text-xs font-semibold uppercase tracking-[0.14em] text-[#4b7f08]">View all</Link>}><DashboardActivity items={activity} empty="Your introduction and connection activity will appear here." /></DashboardPanel></div>
      ...
      <div className="grid gap-4 md:grid-cols-3"><DashboardPanel title="Privacy & visibility" eyebrow="Control your profile"><p className="mt-4 text-sm text-[#08111F]/60">Control who can discover you and what employers can see.</p><DashboardAction href="/settings/privacy" variant="green">Manage privacy</DashboardAction></DashboardPanel><DashboardPanel title="Free Agent Pro" eyebrow="Subscription">{props.isProTalent ? <><p className="mt-4 text-sm text-[#08111F]/60">Your Pro subscription is active.</p><BillingButton action="portal" className="dashboard-upgrade-pill">Manage subscription <span aria-hidden="true">→</span></BillingButton></> : <><p className="mt-4 text-sm text-[#08111F]/60">Unlock additional visibility insights and Pro features.</p><BillingButton action="checkout" plan="free_agent_pro" className="dashboard-upgrade-pill">Upgrade to Pro <span aria-hidden="true">→</span></BillingButton></>}</DashboardPanel><DashboardPanel title="Sign out" eyebrow="Account"><p className="mt-4 text-sm text-[#08111F]/60">Finished for now?</p><button type="button" onClick={props.signOut} className="dashboard-action dashboard-action-outline">Sign out <span aria-hidden="true">→</span></button></DashboardPanel></div>
